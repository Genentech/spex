...
_LEFT_FILENAME = sys.argv[1]
_RIGHT_FILENAME = sys.argv[2]
_PNG_FILENAME = sys.argv[3]
_MATCH_RESULT_FILENAME = sys.argv[4]

# Minimal bin size for binning the mix.
_BIN_SIZE = int(sys.argv[5])
...
x_column_list = sys.argv[7].split(',')
_DATA_FILES_X_COLUMNS = [int(x) for x in x_column_list]
# In which column we have cluster id in data files.
_DATA_FILES_CLUSTER_ID_COLUMN = int(sys.argv[6])
...
class _Matcher(object):
  ...
  def _MixAndBin(self):
    ...
    # And bin the medley.
    good_binner = SplittingInHalfBinner(
  ...
  def _CalculateDissimilarities(self):
    ...
    for left_cluster_id, left_bin_collection in (iter(self
        ._left_bin_collection_by_cluster_id.items())):
      for right_cluster_id, right_bin_collection in (iter(self
          ._right_bin_collection_by_cluster_id.items())):
        if (
          _IsWithin(
...
def main(unused_argv):
  ...
  print('left_lists length: {0}; right_lists length: {1}'.format(len(left_lists),len(right_lists)))
